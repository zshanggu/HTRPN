...
__all__ = ["GeneralizedRCNN", "TernaryHiercGeneralizedRCNN", "ProposalNetwork"]
...
class GeneralizedRCNN(nn.Module):
    ...
    def forward(self, batched_inputs, alg_iter=0):
        """
        Args:
        ...
        """
        # print("rcnn.py alg_iter...", alg_iter)
        ...
        _, detector_losses = self.roi_heads(images, features, proposals, gt_instances, alg_iter=alg_iter)
...
@META_ARCH_REGISTRY.register()
class TernaryHiercGeneralizedRCNN(nn.Module):
    """
    Generalized R-CNN. Any models that contains the following three components:
    1. Per-image feature extraction (aka backbone)
    2. Region proposal generation
    ...
    def forward(self, batched_inputs, alg_iter=0):
        ...
        # backbone FPN
        # print("batched_inputs!!!!!!!!!!!!!!!!!!!!!!!!!!", batched_inputs[0]['file_name'])
        images = self.preprocess_image(batched_inputs)
        features = self.backbone(images.tensor)  # List of L, FPN features
        ...
        if self.proposal_generator:
            ...
            proposals, for_calculate_loss = self.proposal_generator(images, features, gt_instances, batched_inputs)
            # print("proposals!!!!!!!!!!!!!!", proposals[0]._fields['proposal_boxes'].tensor)
            # print("rcnn.py proposals!!!!!!!!!!!!!!", proposals)  # IMS_PER_BATCH / num_gpus = images, 2000 instances per image
            # proposals is the output of find_top_rpn_proposals(), i.e., post_nms_top_K
        else:
            assert "proposals" in batched_inputs[0]
            proposals = [x["proposals"].to(self.device) for x in batched_inputs]
        ...
        # ROI inputs are post_nms_top_k proposals.
        # detector_losses includes Contrast Loss, 和业务层的 cls loss, and reg loss
        # print("proposals......", proposals)  # IMS_PER_BATCH / num_gpus = images, 2000 instances per image
        _, detector_losses, proposal_losses = self.roi_heads(images, features, proposals, gt_instances, alg_iter=alg_iter, for_calculate_loss=for_calculate_loss, batched_inputs=batched_inputs)
        # print("rcnn.py...... _", _)
        # print("rcnn.py...... detector_losses", detector_losses)
