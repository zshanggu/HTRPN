...
class PascalVOCDetectionEvaluator(DatasetEvaluator):
    ...
    def evaluate(self):
        ...
        self._logger.info("Evaluate all std-div:"+str(np.std(aps[50])))
        self._logger.info("Evaluate novel std-div:"+str(np.std(aps[50][-5:])))
...
def voc_eval(detpath, annopath, imagesetfile, classname, ovthresh=0.5, use_07_metric=False):
    ...
    for imagename in imagenames:
        ...
        difficult = np.array([x["difficult"] for x in R]).astype(bool_)
        # difficult = np.array([False for x in R]).astype(bool_)  # treat all "difficult" as GT
        det = [False] * len(R)
